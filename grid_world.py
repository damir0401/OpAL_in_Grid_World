...
class GridWorld:
    ...
    def __init__(self, tot_row, tot_col):
        ...
        self.position = [tot_row - 1, 0] 
        self.reward_prob_matrix = np.zeros((tot_row, tot_col))
    ...
    def setRewardProbabilityMatrix(self, reward_prob_matrix):
        '''Set the reward probability matrix.
    ...
    def step(self, action):
        ...
        #from the trasition model it chooses a new action to perform
        ...
        reward = self.reward_matrix[self.position[0], self.position[1]] if (rnd_num < self.reward_prob_matrix[self.position[0], self.position[1]]) else 0
        
        #Done is True if the state is a terminal state
        done = bool(self.state_matrix[self.position[0], self.position[1]])
        in_worst_term_state = (self.position[0] == 1) & (self.position[1] == 3)
        return self.position, reward, done, in_worst_term_state
